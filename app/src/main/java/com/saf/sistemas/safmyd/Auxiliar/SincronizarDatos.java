package com.saf.sistemas.safmyd.Auxiliar;
...
import static com.saf.sistemas.safmyd.Auxiliar.Funciones.Codificar;
...
public class SincronizarDatos {
    ...
    public SincronizarDatos(DataBaseOpenHelper dbh, ClientesHelper Clientesh,
                            ...
                            ConfiguracionSistemaHelper ConfigSistemah,
                            ClientesSucursalHelper ClientesSuch, ArticulosHelper Articulosh, UsuariosHelper usuariosH,
                            FacturasHelper facturasH, FacturasLineasHelper facturasLineasH , TPreciosHelper tpreciosH,
                            RutasHelper rutasH, EscalaPreciosHelper escalaPreciosH,RecibosHelper recibosH,
                            FacturasPendientesHelper facturasPendientesH,PreciosHelper PrecioEspecialh) {
        DbOpenHelper = dbh;
        ClientesH = Clientesh;
        VendedoresH = Vendedoresh;
        PromocionesH = Promocionesh;
    ...
    public SincronizarDatos(DataBaseOpenHelper dbh, ClientesHelper Clientesh,
                            ...
                            FacturasHelper facturash, FacturasLineasHelper facturaslineash,RecibosHelper recibosH,FacturasPendientesHelper facturasPendientesH,PreciosHelper PrecioEspecialh) {
        DbOpenHelper = dbh;
        ClientesH = Clientesh;
        VendedoresH = Vendedoresh;
        PromocionesH = Promocionesh;
    ...
    public boolean SincronizarClientes() throws JSONException {
        ...
        try {
            ...
            for (int i = 0; i < clientes.length(); i++) {
                ...
                String Saldo = c.getString("SaldoActual");
                String NoenRuta = c.getString("NoEnRuta");
                String Lat = c.getString("Latitud");
                String Lng = c.getString("Longitud");
                String Vs = c.getString("Visita");
                ClientesH.GuardarClientes(IdCliente, Nombre,  FechaCreacion, Telefono, Direccion, IdDepartamento, IdMunicipio,
                        ...
                        Cod_Zona,Cod_SubZona,Pais_Id,Pais_Nombre,IdTipoNegocio,TipoNegocio,Saldo,NoenRuta,Lat,Lng,Vs);
            }
            DbOpenHelper.database.setTransactionSuccessful();
            return true;
        } catch (Exception ex) {
   ...
   public boolean SincronizarRutas() throws JSONException {
    ...
    public boolean SincronizarPrecioEspecial() throws JSONException {
        ...
        if (jsonStrPrecioEspecial == null) {
            new Funciones().SendMail("Ha ocurrido un error al Sincronizar PrecioEspecial, Respuesta nula", variables_publicas.info + urlStringPrecioEspecial, "dlunasistemas@gmail.com", variables_publicas.correosErrores);
            return false;
        }

        DbOpenHelper.database.beginTransactionNonExclusive();
        PrecioEspecialH.EliminaPrecios();
        ...
        try {
            ...
            for (int i = 0; i < PrecioEspecial.length(); i++) {
                ...
                PrecioEspecialH.GuardarPrecios(Id, CodigoArticulo, IdCliente, Descuento, Precio, Facturar);
            }
            DbOpenHelper.database.setTransactionSuccessful();
            return true;
        } catch (Exception ex) {
            new Funciones().SendMail("Ha ocurrido un error al Sincronizar PrecioEspecial, Excepcion controlada", variables_publicas.info + ex.getMessage(), "dlunasistemas@gmail.com", variables_publicas.correosErrores);
            return false;
        } finally {
            DbOpenHelper.database.endTransaction();
        }
    ...
    public static String ConsultarExistencia2(String CodigoArticulo) {
        HttpHandler sh = new HttpHandler();
        String encodeUrl = "";

        final String urlConsulta = urlConsultarExistencias + CodigoArticulo + "/01/"+ variables_publicas.usuario.getEmpresa_ID();
    ...
    public static String ConsultarExistencias(final Activity activity, ArticulosHelper ArticulosH, String CodigoArticulo) {
        HttpHandler sh = new HttpHandler();
        String encodeUrl = "";
...
//
        ...
        if (jsonStr != null) {
            ...
            try {
                ...
                for (int i = 0; i < factura.length(); i++) {
                    ...
                    vfacturash.GuardarFactura(c.get("Factura").toString(),c.get("Fecha").toString(),c.get("Cliente").toString(),c.get("Monto").toString(),c.get("SubTotal").toString(),c.get("Descuento").toString(),c.get("Iva").toString(),c.get("Total").toString(),c.get("Vendedor").toString(),c.get("Tipo").toString(),c.get("Tipo_Factura").toString(),c.get("Empresa").toString(),c.get("Usuario").toString(),c.get("Sucursal").toString(),c.get("Ruta").toString(),c.get("Orden").toString(),c.get("Guardada").toString(),c.get("Latitud").toString(),c.get("longitud").toString(),c.get("DireccionGeo").toString());
                }
                return true;
            } catch (Exception ex) {
                Log.e("Error", ex.getMessage());
    ...
    public static String SincronizarRecibo( RecibosHelper RecibosH, String vRecibo, String vSerie) {
        ...
        for (HashMap<String, String> item : reciboDetalle) {
            ...
            item.put("Latitud", item.get("Latitud"));
            item.put("Longitud", item.get("Longitud"));
            item.put("DireccionGeo", item.get("DireccionGeo"));
        }
