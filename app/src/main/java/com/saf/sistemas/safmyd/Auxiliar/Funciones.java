...
public class Funciones {
    ...
    public static  String formateartexto(String texto) throws UnsupportedEncodingException {
   /*     byte[] arrByteForSpanish = texto.getBytes("ISO-8859-1");
...
/*
    ...
    public static boolean TestServerConectivity() {
        HttpURLConnection conn=null;
        try {
            URL url = new URL(variables_publicas.direccionIp+"/ServicioPedidos.svc/ObtenerInventarioArticulo/00001/01/1");
            conn = (HttpURLConnection) url.openConnection();
            conn.setConnectTimeout(20000);
            conn.setReadTimeout(20000);
            conn.setUseCaches(false);
