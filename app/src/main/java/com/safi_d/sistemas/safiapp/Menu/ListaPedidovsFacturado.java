...
public class ListaPedidovsFacturado extends Fragment {
    ...
    private void CargaDatosCombo() {
        ...
        if (variables_publicas.usuario.getTipo().equalsIgnoreCase("Supervisor")){
            vendedores = VendedoresH.ObtenerVendedorxSup(variables_publicas.usuario.getCodigo());
        }else {
    ...
    private void ActualizarLista() {
        try {
            ...
            for (HashMap<String, String> item : listapedidosFacturas) {
                ...
                double TotDevuelto= Double.parseDouble(item.get("TotDevuelto").replace("C$", "").replace(",", ""));
                item.put("TotDevuelto", df.format(TotDevuelto));
            }
            adapter = new SimpleAdapter(
                    getActivity(), listapedidosFacturas,

                    R.layout.resumen_pedido_factura_item, new String[]{"Ruta", "Pedido","Factura","Devolucion","Cliente","TotPedido","TotFactura","TotDevuelto"},
                    new int[]{R.id.lblDetRuta, R.id.lblDetPedido, R.id.lblDetFactura,R.id.lblDetDevolucion, R.id.lblDetCliente, R.id.lblDetTotPedido, R.id.lblDetTotFacturado, R.id.lblDetTotDevuelto}) {
            };
